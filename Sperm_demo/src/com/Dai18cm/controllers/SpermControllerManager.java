...
public class SpermControllerManager extends ControllerManager {//Singleton
    ...
    public void run() {
        ...
        if(GameConfig.getInst().durationInMiliSeconds(count) > n){
            ...
            this.singleControllerVector.add(SpermController.create(random_X, 10));
        }
    }
    public void slowAll(){
        for(SingleController singleController : this.singleControllerVector){
            ((SpermController)singleController).setIsInBuff(true);
//            ((SpermController)singleController).setSpeed();
        }
    }

    public void reset(){
        for(SingleController singleController : this.singleControllerVector){
            ((SpermController)singleController).setIsInBuff(false);
        }
    }
    ...
    public void levelChange(LevelType levelType) {
        switch (levelType){
            case LEVEL_1:
                SPEED_RANDOM = 1000;
                SPACE_RANDOM = 100;
                SpermController.LEVEL_SPEED = 6;
                break;
            case LEVEL_2:
                SPEED_RANDOM = 800;
                SPACE_RANDOM = 80;
                SpermController.LEVEL_SPEED = 8;
                break;
            case LEVEL_3:
                SPEED_RANDOM = 500;
                SPACE_RANDOM = 50;
                SpermController.LEVEL_SPEED = 10;
                break;
        }
    }
}
